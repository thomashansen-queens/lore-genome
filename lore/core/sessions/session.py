...
class Session(AbstractContextManager):
    ...
    def name(self, value: str) -> None:
        ...
        # Session is closed — open it to update the manifest via normal machinery
        else:
    ...
    def display_size(self) -> str:
        ...
        # Session is open
        if self._manifest:
            b = self._manifest.size_bytes or 0
        else:
            # Session is closed, read from disk
            manifest_path = self._root / "manifest.json"
            if manifest_path.exists():
                try:
                    b = json.loads(
    ...
    def add_task(
        ...
        from lore.core.tasks import Task, task_registry, TaskStatus
        ...
        task.update()  # auto-set DRAFT vs READY
        self.mark_dirty()
