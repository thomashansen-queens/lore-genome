"""
Routes for inspecting and managing the cache.
"""
...
def view_cache_overview(rt: RT, ctx: PageContext = Depends()):
    ...
    ctx.generate_breadcrumbs({"cache": "Cache"})

    raw = rt.cache.get_stats()
    stats = {
        "l1_count": raw["l1_count"],
        "l1_usage": f"{_fmt_bytes(raw['l1_bytes'])} / {_fmt_bytes(raw['l1_max_bytes'])}",
        "l1_pct": round(raw["l1_bytes"] / raw["l1_max_bytes"] * 100, 1) if raw["l1_max_bytes"] else 0,
        "l2_count": raw["l2_count"],
        "l2_usage": f"{_fmt_bytes(raw['l2_bytes'])} / {_fmt_bytes(raw['l2_max_bytes'])}",
...
def view_cache_item(key: str, rt: RT, ctx: PageContext = Depends(), inspect: bool = False):
    ...
    ctx.generate_breadcrumbs({"cache": "Cache", "view": "View"})

    item = {
        "key": raw["key"],
        "path": raw["path"],
        "size": _fmt_bytes(raw["size_bytes"]),
...
def clear_cache(rt: RT, ctx: PageContext = Depends()):
    ...
    return ctx.redirect("/cache", message="Cache cleared.", message_type="success")
